      ...
      projects: [
        {
          ...
          contributions: ["Next.js 기반 프론트엔드 아키텍처 설계 및 기술환경 구성", "4개 CSP SDK 데이터를 통합 처리하는 공통 인터페이스 설계 및 구현", "VM 관리, 대시보드, 모니터링 등 핵심 CSP 서비스 전반 UI 개발"],
          techStack: ["TypeScript", "Next.js", "React", "styled-components", "Github Action"],
        },
        {
          ...
          contributions: ["Jest 기반 유닛 테스트 환경 구축 및 설정 표준화", "핵심 비즈니스 로직, 유틸리티 함수, 컴포넌트 테스트 코드 작성", "테스트 커버리지 측정 및 리포팅 체계 수립", "테스트 코드 작성 가이드라인 및 Best Practice 문서화", "CI/CD 파이프라인에 자동화된 테스트 프로세스 통합"],
        ...
        },
        {
          ...
          contributions: ["pre-commit 훅에 ESLint, Prettier를 통한 코드 린팅 및 포맷팅 자동 검증 설정", "pre-push 훅에 Jest 테스트 자동 실행 기능 추가로 버그 코드의 원격 저장소 푸시 방지", "빌드 프로세스 자동화를 통한 배포 전 코드 검증 파이프라인 구성"],
          techStack: ["Husky"],
        },
        {
          ...
          contributions: ["Storybook으로 재사용 가능한 UI 컴포넌트 라이브러리 체계화", "AWS S3 정적 호스팅으로 실시간 접근 가능한 환경 배포", "GitHub Actions를 활용한 PR 머지 시 자동 배포 파이프라인 구축", "컴포넌트별 Props 문서화 및 사용 가이드 작성으로 팀 내 개발 표준 확립"],
        ...
        },
        {
          ...
          contributions: ["Git 커밋 메시지 컨벤션 및 브랜치 전략(Git Flow) 수립", "프론트엔드 코드 컨벤션 및 폴더 아키텍처 가이드라인 작성", "개발 환경 설정 및 배포 프로세스 Step-by-Step 문서화", "PR 템플릿, 이슈 템플릿 등 팀 협업 워크플로우 프로세스 정립"],
        ...
        },
        {
          ...
          contributions: ["템플릿 기반 자동 페이지 생성: 목록 페이지(page.tsx)와 생성 페이지(create/page.tsx) 템플릿 구축", "경로 선택 시스템: Root, AI, Computing, Network 등 경로 옵션을 통한 유연한 페이지 배치", "Mock 데이터 통합: 개발 초기 단계에서 즉시 테스트 가능한 가짜 데이터함수 자동 생성"],
          techStack: ["Plop.js"],
        },
        {
          title: "B2B 데이터 마켓 서비스",
          period: "2023.05 - 2024.08",
          description: "기업 간 데이터 거래를 위한 온라인 마켓플레이스 신규 구축 프로젝트입니다. 프론트엔드 개발자로서 판매자의 데이터 상품 등록부터 구매자의 검색, 결제까지 전체 구매 플로우 UI를 구현했습니다.  NestJS 기반 일부 백엔드 API 개발에도 참여하여 풀스택 역량을 확장했습니다.",
          contributions: ["상품 상세 페이지 UI 및 데이터 미리보기 기능 구현", "기업 인증 프로세스 및 결제 화면 개발", "NestJS 기반 일부 백엔드 API 개발 참여"],
          techStack: ["TypeScript", "Next.js", "Styled Component", "Github Action", "AWS(S3 / CF / Route53 / Certificate Manager)", "NestJS"],
        },
      ],
    },
  ],
      ...
      projects: [
        {
          ...
          description: "AWS 정적 웹사이트 배포 시 S3 업로드, CloudFront 설정, 캐시 무효화 등 15분 걸리는 반복적인 수동 작업을 TypeScript CLI로 자동화했습니다. 파일 해시 비교를 통한 증분 배포로 배포 시간을 2분으로 87% 단축시켰으며, npm 패키지로 배포 완료하고 GitHub 오픈소스화를 준비 중입니다.",
          contributions: [
          ...
          techStack: ["TypeScript", "Commander.js", "AWS SDK v3", "Zod", "Husky", "Github Actions"],
        },
      ],
    },
    {
      ...
      projects: [
        {
          ...
          techStack: ["TypeScript", "Next.js", "TailwindCSS", "Framer Motion", "Zustand", "Firebase", "Kakao SDK", "AWS (S3 / CloudFront)", "Husky", "Github Actions"],
        },
      ],
    },
    {
      ...
      projects: [
        {
          ...
          contributions: ["Next.js 기반 프론트엔드 인터페이스 및 편향성 시각화 컴포넌트 구현", "Python + Ollama를 활용한 로컬 LLM 기반 뉴스 분석 시스템 구축", "키워드 분석과 AI 판단을 결합한 편향성 점수 알고리즘 설계", "Monorepo 구조로 프론트엔드/백엔드 통합 관리 및 RESTful API 개발"],
          techStack: ["TypeScript", "Next.js", "Emotion", "Python", "Ollama AI", "Monorepo", "AWS(S3 / CF / Route53 / Certificate Manager / EC2)"],
        },
      ],
    },
    {
      ...
      projects: [
        {
          ...
          contributions: ["Next.js 기반 프론트엔드 전체 UI 구현", "관계형 데이터베이스 ERD 설계 및 NestJS 서버 구축", "RESTful API 설계 및 구현, Swagger를 활용한 API 명세 문서화", "Nginx 웹서버 구성 및 AWS 인프라 배포"],
          techStack: ["TypeScript", "Next.js", "React", "styled-components", "AWS(S3 / CF / Route53 / Certificate Manager / EC2)", "NestJS", "Nginx"],
        },
      ],
    },
  ],
  ...
  skills: [
    {
      ...
      details: [
        {
          title: "HTML/CSS",
          description: ["Semantic Markup을 신경쓰며 코드 가독성을 향상시키는 데 노력합니다.", "CSS를 최대한 간단하고, 개발자 친화적으로 짤 수 있게 노력합니다.", "다양한 CSS 방식을 사용해보았습니다.", "다양한 UI 라이브러리 사용 경험이 있습니다."],
          additionalInfo: [
    ...
    },
    {
      ...
      details: [
        {
          title: "Backend & Infra",
          description: ["Node.js와 NestJS를 활용한 백엔드 개발 경험이 있습니다.", "RESTful API 설계 및 구현에 경험이 있습니다.", "AWS 서비스(S3, CloudFront, Route53, EC2 등)를 활용한 경험이 있습니다.", "CI/CD 파이프라인 구축 및 자동화된 배포 프로세스 경험이 있습니다."],
          additionalInfo: [
